...
from ..base import KLine, OrderInfo, PlaceOrderContext, Runner
...
class ETH5MRunner(Runner):
    ...
    async def _choice_order_block_extra(
        ...
        if (
            elapsed < self.effective_start_time
            or elapsed > self.effective_end_time
        ):
        ...
        if volume_percent < self.volume_percent_threshold:
            message.append(
                f"[成交量比例] {volume_percent} < {self.volume_percent_threshold}."
            )
    ...
    async def _resolve_order_info(
        ...
        if order_block.side == "long":
            ...
            preset_stop_surplus_price = (
                price + kline.delta_price * self.profit_and_loss_ratio
            )
        else:
            # 空单入场在下影线
